...
dataset_train = Dataset(data=dec_train, k=3, num_classes=3, T=10)
dataset_val = Dataset(data=dec_val, k=3, num_classes=3, T=10)
dataset_test = Dataset(data=dec_test, k=3, num_classes=3, T=10)
...
#             nn.GroupNorm(1, in_chnls),  # layer norm
            nn.GroupNorm(1, in_chnls),  # nn.GroupNorm(num of groups, num of channels)
...
#             nn.GroupNorm(1, self.group_chnls),  # layer norm
            nn.GroupNorm(2, self.group_chnls),
            nn.LeakyReLU(negative_slope=0.01),
...
#             nn.GroupNorm(1, channel),  # layer norm
            nn.GroupNorm(4, channel),
            nn.LeakyReLU(negative_slope=0.01),
...
def batch_gd(model, criterion, optimizer, train_loader, test_loader, epochs):
    ...
    for it in tqdm(range(epochs)):  # 進度條
        ...
        if test_loss < best_test_loss:
            torch.save(model, './best_val_model_pytorch_6')
...
model = torch.load('best_val_model_pytorch_6')
#sample_input = torch.ones((1, 1, 10, 40)).cuda()
